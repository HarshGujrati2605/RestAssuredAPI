...
public class AddBookPostAPIDataProviderWithPOJO extends Objects {
	...
	public void addbook(String aisle, String isbn) {
		...
		Response response = given().log().all().spec(libraryspecbuilder.librarySpecBuilder()).body(m).when()
				.post("/Library/Addbook.php").then().log().all().statusCode(200).extract().response();

		GlobalVariable.book_idlist.add(JsonConversionUtilities.getRawToJsonData(response.asString(), "ID"));
	...
	public void getCreatedLibraryBookValidation() {
		...
		for (int i = 0; i < GlobalVariable.book_idlist.size(); i++) {
			...
			String response = given().spec(libraryspecbuilder.librarySpecBuilder())
					.queryParam("ID", "" + GlobalVariable.book_idlist.get(i) + "").log().all().when().get().then()
...
//			System.out.println(json.getList("isbn").get(0)+ "Actual response isbn");
			System.out.println(JsonConversionUtilities.getArrayRawToJsonData(response, "isbn"));
			assertEquals(JsonConversionUtilities.getArrayRawToJsonData(response, "isbn"),
					GlobalVariable.isbnlist.get(i));
